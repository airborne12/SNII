...
std::string TempPath() {
  ...
  return "/tmp/snii_e2e_test_" + std::to_string(getpid()) + "_" +
...
Corpus BuildCorpus() {
  ...
  for (uint32_t d = 0; d < c.doc_count; ++d) {
    ...
    if (d % 7 == 0) {
      toks.push_back("quick");
      toks.push_back("brown");
      toks.push_back("fox");
      toks.push_back("jumps");
      toks.push_back("over");
    }
    ...
    if (d % 11 == 0) toks.push_back("rare");
  }

  for (uint32_t d = 0; d < c.doc_count; ++d) {
    for (const auto& t : c.docs[d]) c.term_docs[t].insert(d);
  }
  return c;
}
...
TEST(SniiEndToEnd, TermAndPhraseAgainstOracle) {
  ...
  std::vector<std::string> present_terms = {
      "the",  "quick", "brown", "fox",  "jumps", "over",
      "lazy", "dog",   "rare",  "alpha", "ubiquitous"};
  ...
  // ---- metrics sanity: a fresh phrase query plans/batches its reads. ----
  metered.reset_metrics();
  std::vector<uint32_t> got;
  ASSERT_TRUE(query::phrase_query(
                  idx, {"quick", "brown", "fox", "jumps", "over"}, &got)
                  .ok());
  const auto& m = metered.metrics();
  EXPECT_GE(m.serial_rounds, 1u);
  EXPECT_GE(m.range_gets, 1u);
  // A 5-term phrase must not serialize one round per posting list; the planned
  // batch should keep rounds small (DICT block reads + one batched postings
  // read). Bound generously but well below "proportional to postings".
