...
bool HasPrefix(const std::string& term, const std::string& prefix) {
  return term.size() >= prefix.size() && term.compare(0, prefix.size(), prefix) == 0;
}
...
Corpus BuildPhraseCorpus() {
  Corpus c;
  c.docs = {
      {"quick", "brown", "fox"}, {"quick", "blue", "fox"}, {"quick", "bronze", "fox"},
...
Corpus BuildWideTailCorpus() {
  ...
  for (uint32_t d = 0; d < c.docs.size(); ++d) {
    c.docs[d].push_back(d == 0 ? "lead" : "other");
    char term[16];
...
TEST(PhrasePrefixQuery, MatchesPositionOracle) {
...
TEST(PhrasePrefixQuery, WideTailPrefixAvoidsPerExpansionLookup) {
...
TEST(PhrasePrefixQuery, WideTailPrefixReusesExactTermPostingReads) {
