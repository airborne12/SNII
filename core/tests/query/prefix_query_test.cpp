...
std::string TempPath() {
  ...
  return "/tmp/snii_prefix_query_" + std::to_string(getpid()) + "_" +
  ...
  std::vector<uint32_t> prefix_docs(const std::string& prefix) const {
    ...
    for (uint32_t d = 0; d < docs.size(); ++d) {
      for (const std::string& term : docs[d]) {
        if (term.size() >= prefix.size() && term.compare(0, prefix.size(), prefix) == 0) {
          ids.insert(d);
        }
      }
    }
...
TEST(PrefixQuery, MatchesPrefixOracle) {
  ...
  for (const char* prefix : {"aa_", "aa_e", "ab_0", "zz_", "missing"}) {
    ...
    EXPECT_TRUE(std::is_sorted(got.begin(), got.end())) << prefix;
...
TEST(PrefixQuery, UsesEnumeratedEntriesWithoutPerTermLookup) {
