...
std::string TempPath() {
  ...
  return "/tmp/snii_score_test_" + std::to_string(getpid()) + "_" +
...
Corpus MakeCorpus() {
  ...
  for (uint32_t d = 0; d < c.doc_count; ++d) {
    ...
    if (d % 2 == 0) add("common", d, 1 + (d % 4));
    // "rare": appears in only a few docs.
    if (d == 3 || d == 17 || d == 42) add("rare", d, 2);
...
std::vector<ScoredDoc> ReferenceRanking(const Corpus& c,
  ...
  for (const auto& term : terms) {
    ...
    const double idf = std::log(1.0 + (static_cast<double>(c.doc_count) - df + 0.5) /
                                          (df + 0.5));
...
TEST(ScoringQuery, ReferenceOracleAndWandEqualsExhaustive) {
  ...
  uint64_t sum_ttf = 0;
  for (const auto& dl : corpus.doc_len) sum_ttf += dl;
  EXPECT_EQ(stats.indexed_doc_count(), corpus.doc_count);
  EXPECT_EQ(stats.sum_total_term_freq(), sum_ttf);
  EXPECT_NEAR(stats.avgdl(),
              static_cast<double>(sum_ttf) / corpus.doc_count, 1e-9);
  ...
  for (const auto& [term, plist] : corpus.postings) {
    ...
    uint64_t exp_ttf = 0;
    for (const auto& [d, f] : plist) exp_ttf += f;
  ...
  auto run_and_check = [&](const std::vector<std::string>& terms) {
    ...
    ASSERT_TRUE(snii::query::scoring_query_exhaustive(idx, stats, terms, k, params,
                                                      &exhaustive)
                    .ok());
    std::vector<ScoredDoc> wand;
    ASSERT_TRUE(
        snii::query::scoring_query_wand(idx, stats, terms, k, params, &wand).ok());
...
Corpus MakeWindowedTieCorpus() {
  ...
  for (uint32_t d = 0; d < c.doc_count; ++d) {
    ...
    if (d % 2 == 0) add("evenz", d, 1);  // df=350 (windowed), another high-df term
...
TEST(ScoringQuery, WandEqualsExhaustiveWithTiesAndWindowedTerms) {
  ...
  auto check = [&](const std::vector<std::string>& terms, uint32_t k) {
    ...
    for (size_t i = 0; i < ex.size(); ++i) {
      EXPECT_EQ(wa[i].docid, ex[i].docid) << "terms[0]=" << terms[0] << " k=" << k << " i=" << i;
  ...
  for (uint32_t k : {1u, 3u, 5u, 50u, 200u}) check({"anchor"}, k);
  // Two windowed terms: even docs score higher (two terms) -> ties within each tier.
  for (uint32_t k : {1u, 4u, 10u, 100u}) check({"anchor", "evenz"}, k);
