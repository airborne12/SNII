...
std::string TempPath() {
  ...
  return "/tmp/snii_pattern_query_" + std::to_string(getpid()) + "_" +
...
Corpus BuildMixedCorpus() {
  ...
  for (uint32_t d = 0; d < c.doc_count; ++d) {
    ...
    if (d < 80 && d % 2 == 0) terms.emplace_back("aa_even");
    if (d < 50) {
      char term[16];
...
TEST(PatternQuery, WildcardMatchesOracle) {
  ...
  for (const char* pattern : {"aa_*", "aa_00?", "aa_even", "zz_0??", "missing*"}) {
    ...
    EXPECT_TRUE(std::is_sorted(got.begin(), got.end())) << pattern;
...
TEST(PatternQuery, RegexpMatchesOracle) {
  const Corpus corpus = BuildMixedCorpus();
  const std::string path = TempPath();
  WriteCorpus(corpus, path);
...
TEST(PatternQuery, WideWildcardUsesPrefixEnumerationWithoutPerTermLookup) {
