...
std::string TempPath() {
  ...
  return "/tmp/snii_byte_skip_" + std::to_string(getpid()) + "_" +
...
uint32_t HiFreq(uint32_t occ) { return 40u + (occ * 13u + 7u) % 200u; }
...
Corpus BuildCorpus() {
  ...
  for (uint32_t d = 0; d < c.doc_count; ++d) {
    ...
    if (d % 97 == 0) toks.push_back("aa_rare");
...
TEST(ByteSkip, DocidAndPhraseSkipFreqWhileScoringKeepsIt) {
  ...
  ASSERT_LT(hi.docs, hi.full)
      << "docs-only prefix not smaller: docs=" << hi.docs << " full=" << hi.full;
  ...
  EXPECT_EQ(saved, hi.full - hi.docs)
      << "skipped bytes != freq-region size: saved=" << saved
  ...
  ASSERT_TRUE(query::scoring_query_exhaustive(idx, stats, score_terms, kTopK, params,
                                              &exhaustive)
                  .ok());
  ASSERT_TRUE(
      query::scoring_query_wand(idx, stats, score_terms, kTopK, params, &wand).ok());
  ...
  ASSERT_TRUE(
      query::scoring_query_exhaustive(idx, stats, {"aa_hi"}, kTopK, params, &hi_only)
          .ok());
  ...
  EXPECT_GE(score_frq_request, term_frq_request + (hi.full - hi.docs))
      << "scoring did not read the full freq region: scoring="
      << score_frq_request << " docid=" << term_frq_request
      << " freq_region=" << (hi.full - hi.docs);
  EXPECT_GT(score_frq_request, term_frq_request)
      << "scoring should read MORE .frq bytes than docid-only: scoring="
      << score_frq_request << " docid=" << term_frq_request;
