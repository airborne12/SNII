...
std::string TempPath() {
  ...
  return "/tmp/snii_query_profile_" + std::to_string(getpid()) + "_" +
...
Corpus BuildCorpus() {
  ...
  for (uint32_t d = 0; d < c.docs.size(); ++d) {
    ...
    doc.push_back("lead");
    doc.push_back("quick");
    doc.push_back(d % 2 == 0 ? "brown" : "bronze");
    char term[16];
...
void ExpectProfileMatchesMeteredDelta(
...
TEST(QueryProfile, ReportsElapsedTimeAndMeteredIoForNativeOperators) {
...
TEST(QueryProfile, ReportsElapsedTimeForOperatorErrorPath) {
  const Corpus corpus = BuildCorpus();
  const std::string path = TempPath();
  WriteCorpus(corpus, path);
...
TEST(QueryProfile, ScopeFinalizesProfileOnEarlyReturn) {
  query::QueryProfile profile;
  {
