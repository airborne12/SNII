...
std::string TempPath() {
  ...
  return "/tmp/snii_wand_sel_" + std::to_string(getpid()) + "_" +
...
Corpus MakeVariedCorpus(uint32_t doc_count) {
  ...
  for (uint32_t d = 0; d < doc_count; ++d) {
    ...
    if (d % 2 == 0) add("aa_evenmid", d, 1 + (d % 3));
    if (d % 3 == 0) add("aa_thirdmid", d, 1 + (d % 4));
    if (d % 37 == 0) add("aa_rarea", d, 2);
    if (d % 53 == 0) add("aa_rareb", d, 3);
    if (d % 101 == 0) add("aa_rarec", d, 1);
...
Corpus MakeTieCorpus(uint32_t doc_count) {
  ...
  for (uint32_t d = 0; d < doc_count; ++d) {
    ...
    if (d % 2 == 0) add("aa_evenz", d, 1);    // another high-df windowed term
    if (d % 41 == 0) add("aa_rarez", d, 1);   // low-df, still freq=1
...
std::vector<ScoredDoc> ReferenceRanking(const Corpus& c,
  ...
  uint64_t sum_ttf = 0;
  for (const auto& dl : c.doc_len) sum_ttf += dl;
  const double avgdl =
...
struct OpenedIndex {
  ...
  explicit OpenedIndex(const std::string& path, size_t block_size)
      : metered(&inner, block_size) {
...
uint32_t WindowCount(const LogicalIndexReader& idx, const std::string& term) {
  ...
  if (!found || entry.enc != DictEntryEnc::kWindowed) return 0;
...
void ExpectValidTopK(const std::vector<ScoredDoc>& a, const std::vector<ScoredDoc>& b,
  ...
  while (i < a.size()) {
    size_t j = i + 1;
    while (j < a.size() && std::abs(a[j].score - a[i].score) <= tol) ++j;
...
TEST(ScoringWandSelective, MatchesExhaustiveOverRandomQueries) {
  ...
  const std::vector<std::string> vocab = {
      "aa_hi",    "aa_evenmid", "aa_thirdmid", "aa_rarea",   "aa_rareb",
      "aa_rarec", "aa_pad0",    "aa_pad7",     "aa_missing"};
  ...
  for (int iter = 0; iter < 300; ++iter) {
    ...
    ASSERT_TRUE(snii::query::scoring_query_wand_selective(oi.idx, oi.stats, terms,
                                                          k, params, &sel)
                    .ok());
    ASSERT_TRUE(snii::query::scoring_query_exhaustive(oi.idx, oi.stats, terms, k,
                                                      params, &ex)
                    .ok());
    ASSERT_TRUE(
        snii::query::scoring_query_wand(oi.idx, oi.stats, terms, k, params, &wa)
            .ok());
    const std::vector<ScoredDoc> ref =
        ReferenceRanking(corpus, norms, terms, k, params);

    std::string label = "iter " + std::to_string(iter) + " k=" +
                        std::to_string(k) + " terms:";
...
TEST(ScoringWandSelective, MatchesExhaustiveWithTies) {
  ...
  for (uint32_t k : {1u, 2u, 3u, 5u, 25u, 200u, 1000u}) check({"aa_hi"}, k);
  for (uint32_t k : {1u, 4u, 10u, 100u, 800u}) check({"aa_hi", "aa_evenz"}, k);
  for (uint32_t k : {1u, 5u, 50u}) check({"aa_hi", "aa_evenz", "aa_rarez"}, k);
  std::remove(path.c_str());
}
...
TEST(ScoringWandSelective, FetchesFewerWindowsForSmallKHighDf) {
  ...
  ASSERT_TRUE(snii::query::scoring_query_wand_selective(oi.idx, oi.stats, terms, k,
                                                        params, &sel)
                  .ok());
  ...
  ASSERT_TRUE(read_windowed_posting(oi.idx, entry, frq_base, prx_base,
                                    /*want_positions=*/false, /*want_freq=*/true,
                                    &dp)
                  .ok());
  ...
  EXPECT_LT(sel_m.total_request_bytes, full_m.total_request_bytes)
      << "sel=" << sel_m.total_request_bytes
      << " full=" << full_m.total_request_bytes;
