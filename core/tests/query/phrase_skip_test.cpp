...
//
// Builds a corpus with a VERY high-df term ("aa_hi", df=2500) whose .frq posting
// spans MANY 256-doc windows, plus low/mid-df terms, and plants known phrases
// (including a 5-term phrase whose FIRST term is the high-df term). For every
// query the skipping phrase_query result must equal:
...
std::string TempPath() {
  ...
  return "/tmp/snii_phrase_skip_" + std::to_string(getpid()) + "_" +
...
Corpus BuildCorpus() {
  ...
  for (uint32_t d = 0; d < c.doc_count; ++d) {
    ...
    if (is_hi && occ < kPhraseSpan) {
      // 5-term phrase led by the high-df term, concentrated in the first windows.
...
// for slim/inline) -> sorted docids + aligned per-doc positions. This mirrors the
...
bool DecodeFullTerm(const LogicalIndexReader& idx, const std::string& term,
                    FullPosting* out) {
  DictEntry entry;
  ...
  Status st = decode_window_slices(meta, Slice(frq), Slice(prx),
                                   /*want_positions=*/true, &out->docids, &freqs,
                                   &out->positions);
...
std::vector<uint32_t> FullReadPhrase(const LogicalIndexReader& idx,
  ...
  for (size_t t = 0; t < phrase.size(); ++t) {
    if (!DecodeFullTerm(idx, phrase[t], &posts[t])) return {};
  }
  ...
  for (uint32_t d : cand) {
    ...
    for (uint32_t start : first) {
      ...
      for (size_t t = 1; t < posts.size(); ++t) {
        ...
        if (!std::binary_search(ps.begin(), ps.end(), start + static_cast<uint32_t>(t))) {
          ok = false;
          break;
        }
      }
...
// Looks up the high-df term and reports its full .frq/.prx length + window count.
void HighDfStats(const LogicalIndexReader& idx, const std::string& term,
                 uint64_t* frq_len, uint64_t* prx_len, uint32_t* windows) {
  DictEntry entry;
  uint64_t frq_base = 0, prx_base = 0;
  bool found = false;
...
TEST(PhraseSkip, SkippingEqualsOracleAndFullRead) {
  ...
  // 3. Range GETs stay small and are NOT proportional to the term's window count
