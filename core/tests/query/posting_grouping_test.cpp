...
// grouping). The windowed .frq payload is laid out [prelude][dd-block][freq-block]
// so a docid-only / phrase reader fetches the docs-only data ([prelude][dd-block])
// as ONE CONTIGUOUS run. Over a ~5000-doc kDocsPositionsScoring index with a
// high-df term spanning MANY adaptive windows plus mid/low terms and a planted
// 5-term phrase, this asserts:
//   (a) term_query + phrase_query (incl the 5-term phrase) docids == a brute-force
//       ORACLE; scoring top-K (exhaustive == wand == selective) unchanged.
//   (b) Through a MeteredFileReader a docid-only term_query on the high-df term
//       reads the dd-block as a CONTIGUOUS region: read_at is SMALL (a couple of
//       ranges, NOT one-per-window / not thousands), range_gets is small, AND
//       request_bytes is strictly LESS than fetching the full posting (the
//       freq-block is skipped). All three (read_at, range_gets, request_bytes)
//       drop vs the full-posting read, with identical docids.
...
// aa_hi appears with varied, large frequency so its per-window freq region is big
// in absolute terms (the freq-block the docs-only path never fetches).
uint32_t HiFreq(uint32_t occ) { return 50u + (occ * 17u + 11u) % 240u; }
...
struct Corpus {
  ...
  std::vector<uint32_t> term_oracle(const std::string& term) const {
    ...
    for (uint32_t d = 0; d < doc_count; ++d) {
      const auto& toks = docs[d];
      if (std::find(toks.begin(), toks.end(), term) != toks.end()) out.push_back(d);
    }
    return out;
  }
  ...
  std::vector<uint32_t> phrase_oracle(const std::vector<std::string>& phrase) const {
    std::vector<uint32_t> out;
    for (uint32_t d = 0; d < doc_count; ++d) {
      if (phrase_in_doc(d, phrase)) out.push_back(d);
    }
    return out;
  }
};
...
Corpus BuildCorpus() {
  ...
  for (uint32_t d = 0; d < c.doc_count; ++d) {
    ...
    if (is_hi) toks.push_back("aa_hi");  // high-df, position 0
    const uint32_t occ = d;

    if (is_hi && occ < kPhraseSpan) {
      toks.push_back("aa_quick");
      toks.push_back("aa_brown");
      toks.push_back("aa_fox");
      toks.push_back("aa_jumps");
    } else if (d % 13 == 0) {
      toks.push_back("aa_lazy");
      toks.push_back("aa_dog");
    }
    if (d % 5 == 0) {
      toks.push_back("aa_mid");
      if (d % 15 == 0) toks.push_back("aa_mid");
    }
    for (uint32_t k = 0; k < 2; ++k) toks.push_back(vocab[(d + k) % vocab.size()]);
    if (d % 101 == 0) toks.push_back("aa_rare");
    char nm[16];
    std::snprintf(nm, sizeof(nm), "zz_%03u", d % 400);
    toks.push_back(nm);
    if (is_hi) {
      const uint32_t extra = HiFreq(occ) - 1u;  // total freq = HiFreq(occ)
      for (uint32_t k = 0; k < extra; ++k) toks.push_back("aa_hi");
    }
...
std::vector<ScoredDoc> ReferenceRanking(const Corpus& c,
                                        const std::vector<std::string>& terms,
                                        uint32_t k, const Bm25Params& params) {
  uint64_t sum_ttf = 0;
  for (uint64_t dl : c.doc_len) sum_ttf += dl;
  const double avgdl =
      static_cast<double>(sum_ttf) / std::max<uint64_t>(1, c.doc_count);
  ...
  for (const auto& term : terms) {
    ...
    for (uint32_t d = 0; d < c.doc_count; ++d) {
      uint32_t f = 0;
      for (const auto& t : c.docs[d]) if (t == term) ++f;
      if (f > 0) plist[d] = f;
    }
    if (plist.empty()) continue;
    const uint64_t df = plist.size();
    const double idf = std::log(
        1.0 + (static_cast<double>(c.doc_count) - df + 0.5) / (df + 0.5));
    for (const auto& [docid, freq] : plist) {
      const double dl =
          snii::query::decode_norm(snii::query::encode_norm(c.doc_len[docid]));
      const double denom =
          freq + params.k1 * (1.0 - params.b + params.b * dl / avgdl);
  ...
  for (const auto& [docid, s] : scores) all.push_back({docid, s});
...
void ExpectRankingEqual(const std::vector<ScoredDoc>& got,
                        const std::vector<ScoredDoc>& ref, const char* label) {
...
// window's dd region in its OWN read round (one BatchRangeFetcher per window), as
// the prior per-window [dd][freq] layout forced -- each window's docs prefix had to
// be fetched on its own because the next window's dd was separated by the current
// window's (skipped) freq region. This reproduces the read_at / range_get explosion
// (one per window) that the grouped, contiguous dd-block eliminates. Same docids.
std::vector<uint32_t> DecodePerWindow(const LogicalIndexReader& idx,
...
TEST(PostingGrouping, ContiguousDdBlockSavesAllThreeMetrics) {
  ...
  // docs-only run occupies strictly fewer cache blocks than (i) the full posting
  // and (ii) a per-window dd fetch whose gaps span the skipped freq regions.
  ...
  const std::vector<std::string> terms_to_check = {"aa_hi", "aa_mid", "aa_quick",
                                                    "aa_lazy", "aa_rare"};
  for (const auto& term : terms_to_check) {
    std::vector<uint32_t> got;
    ASSERT_TRUE(query::term_query(idx, term, &got).ok()) << term;
    EXPECT_TRUE(std::is_sorted(got.begin(), got.end())) << term;
  ...
  // ---- (b) docid-only term_query reads the dd-block CONTIGUOUSLY ------------
  // GROUPED = the Phase-D grouped docid-only path: read prelude + the single
  // contiguous [dd-block] range; the freq-block is skipped on the wire.
  metered.reset_metrics();
  std::vector<uint32_t> grouped_docs;
  ASSERT_TRUE(query::term_query(idx, "aa_hi", &grouped_docs).ok());
  const io::IoMetrics grouped = metered.metrics();
  ASSERT_EQ(grouped_docs, c.term_oracle("aa_hi"));

  // PER-WINDOW = the pre-grouping layout: fetch EACH window's dd region as its own
  // physical range (gaps = the skipped freq regions). Same lookup + prelude.
  metered.reset_metrics();
  const std::vector<uint32_t> perwin_docs = DecodePerWindow(idx, "aa_hi");
  const io::IoMetrics perwin = metered.metrics();
  ASSERT_EQ(perwin_docs, c.term_oracle("aa_hi"));

  // FULL = fetch the whole posting (prelude + dd-block + freq-block). Same lookup.
  metered.reset_metrics();
  {
    DictEntry entry;
    uint64_t fb = 0, pb = 0;
  ...
  // instead of one fetch per window separated by the skipped freq regions.
  ...
  // (2) vs FULL posting: request_bytes and remote_bytes strictly drop (freq-block
  // skipped on the wire and its cache blocks never touched).
