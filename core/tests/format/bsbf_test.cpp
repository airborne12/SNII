...
struct Oracle {
  ...
  void insert(uint64_t h) {
    const uint32_t b = static_cast<uint32_t>(((h >> 32) * nblocks) >> 32);
    uint32_t m[8];
    masks(static_cast<uint32_t>(h), m);
    for (int i = 0; i < 8; ++i) w[b * 8 + i] |= m[i];
  }
  bool find(uint64_t h) const {
    ...
    masks(static_cast<uint32_t>(h), m);
    for (int i = 0; i < 8; ++i)
      if ((w[b * 8 + i] & m[i]) != m[i]) return false;
    return true;
  }
};

std::string tmp_path() {
  return "/tmp/snii_bsbf_" + std::to_string(::getpid()) + "_" +
         std::to_string(std::rand()) + ".bin";
}

TEST(Bsbf, BuildProbeMatchesParquetOracle) {
  ...
  for (uint32_t i = 0; i < n; ++i) {
    ...
    if (b.maybe_contains(h)) ++fp;
  }
  EXPECT_LT(fp, static_cast<uint32_t>(n * 0.05));  // FPR << 5% (target 1%)
}

TEST(Bsbf, SerializeParseOnDemandProbe) {
  ...
  for (uint64_t i = 0; i < prefix; ++i) sink.put_u8(0xAB);
...
TEST(Bsbf, HeaderValidation) {
  ...
  { auto c = bytes; c[0] = 'X'; EXPECT_TRUE(fails(c)); }      // magic
  { auto c = bytes; c[4] = 9; EXPECT_TRUE(fails(c)); }        // version
  { auto c = bytes; c[5] = 1; EXPECT_TRUE(fails(c)); }        // hash strategy
  { auto c = bytes; c[8] ^= 0xFF; EXPECT_TRUE(fails(c)); }    // field -> header crc
  EXPECT_FALSE(BsbfHeader::parse(Slice(bytes.data(), 10), 0, &h).ok());  // short
}

TEST(Bsbf, FastrangeNotMaskVariant) {
