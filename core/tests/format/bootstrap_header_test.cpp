...
TEST(BootstrapHeader, RoundTripDefaults) {
...
TEST(BootstrapHeader, RoundTripNonDefaultFields) {
  BootstrapHeader in{};
...
TEST(BootstrapHeader, EncodeFillsHeaderLength) {
  BootstrapHeader in{};
  ByteSink sink;
  ASSERT_TRUE(encode_bootstrap_header(in, &sink).ok());
  ...
  uint32_t magic_le = static_cast<uint32_t>(bytes[0]) |
...
TEST(BootstrapHeader, WrongMagicRejected) {
  BootstrapHeader in{};
  ByteSink sink;
  ASSERT_TRUE(encode_bootstrap_header(in, &sink).ok());

  auto bytes = sink.buffer();
  bytes[0] ^= 0xFF;  // corrupt the magic; checksum would also fail, but magic check fires first
  BootstrapHeader out{};
  Status s = decode_bootstrap_header(Slice(bytes), &out);
  EXPECT_EQ(s.code(), StatusCode::kCorruption);
}

TEST(BootstrapHeader, ChecksumMismatchRejected) {
  BootstrapHeader in{};
  in.flags = 7;
  ByteSink sink;
  ASSERT_TRUE(encode_bootstrap_header(in, &sink).ok());
  ...
  auto bytes = sink.buffer();
  // Flip a flags byte (offset 8 = after magic(4)+fmt(2)+min(2)); magic stays valid,
  // so only the checksum can detect this.
  bytes[8] ^= 0xFF;
  BootstrapHeader out{};
  Status s = decode_bootstrap_header(Slice(bytes), &out);
  EXPECT_EQ(s.code(), StatusCode::kCorruption);
...
TEST(BootstrapHeader, MinReaderVersionTooNewUnsupported) {
  BootstrapHeader in{};
  // A future writer that demands a reader >= kFormatVersion+1: current reader must refuse.
...
TEST(BootstrapHeader, TruncatedRejected) {
  BootstrapHeader in{};
  ByteSink sink;
  ASSERT_TRUE(encode_bootstrap_header(in, &sink).ok());
...
TEST(BootstrapHeader, EmptyInputRejected) {
  BootstrapHeader out{};
...
TEST(BootstrapHeader, TrailingBytesAfterChecksumRejected) {
  BootstrapHeader in{};
  ByteSink sink;
  ASSERT_TRUE(encode_bootstrap_header(in, &sink).ok());
