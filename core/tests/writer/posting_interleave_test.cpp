...
TEST(PostingInterleave, RoundTripWithPositionsContiguous) {
  ...
  for (uint32_t d = 0; d < 2000; ++d) wide_docs.push_back(d * 2);
  for (uint32_t d = 0; d < 600; ++d) slim_docs.push_back(d * 3 + 1);
  ...
  for (const auto& hit : AllEntries(idx)) {
    ...
    if (e.kind != DictEntryKind::kPodRef) continue;  // INLINE term: nothing in region
...
TEST(PostingInterleave, ByteCorrectnessAndDocsPrefixStaysInFrqSpan) {
  SniiIndexInput in = BaseInput(1, "body", IndexConfig::kDocsPositions);
  in.doc_count = 4096;
  std::vector<uint32_t> wide_docs;
  for (uint32_t d = 0; d < 1500; ++d) wide_docs.push_back(d);
  in.terms.push_back(MakeTerm("aa_wide", wide_docs, /*with_positions=*/true, 2));
  std::vector<uint32_t> slim_docs;
  for (uint32_t d = 0; d < 600; ++d) slim_docs.push_back(d + 5);
  ...
  for (const auto& hit : AllEntries(idx)) {
    const DictEntry& e = hit.entry;
    if (e.kind != DictEntryKind::kPodRef) continue;
...
TEST(PostingInterleave, MultiIndexIndependentPlacements) {
  SniiIndexInput a = BaseInput(1, "body", IndexConfig::kDocsPositions);
  a.doc_count = 2048;
  std::vector<uint32_t> a_docs;
  for (uint32_t d = 0; d < 1000; ++d) a_docs.push_back(d);
  ...
  for (uint32_t d = 0; d < 800; ++d) b_docs.push_back(d * 2);
  ...
  const bool disjoint = (ra.offset + ra.length <= rb.offset) ||
                        (rb.offset + rb.length <= ra.offset);
  EXPECT_TRUE(disjoint) << "ra=[" << ra.offset << "," << ra.length << "] rb=["
                        << rb.offset << "," << rb.length << "]";
...
TEST(PostingInterleave, NoPositionsRegionIsFrqOnly) {
  SniiIndexInput in = BaseInput(1, "tag", IndexConfig::kDocsOnly);
  in.doc_count = 4096;
  std::vector<uint32_t> wide_docs, slim_docs;
  for (uint32_t d = 0; d < 1500; ++d) wide_docs.push_back(d);
  for (uint32_t d = 0; d < 600; ++d) slim_docs.push_back(d + 2);
...
TEST(PostingInterleave, EmptyTermSetTierFromFlag) {
...
TEST(PostingInterleave, InlineBetweenPodRefIsGapless) {
  SniiIndexInput in = BaseInput(1, "body", IndexConfig::kDocsPositions);
  in.doc_count = 4096;
  std::vector<uint32_t> wide1, wide2;
  for (uint32_t d = 0; d < 1500; ++d) wide1.push_back(d);
  for (uint32_t d = 0; d < 1200; ++d) wide2.push_back(d + 1);
  ...
  for (const auto& [term, expect] :
       std::vector<std::pair<std::string, std::vector<uint32_t>>>{
           {"aa_big", wide1}, {"bb_tiny", {3, 8}}, {"cc_big", wide2},
           {"dd_tiny", {1, 99}}}) {
    std::vector<uint32_t> got;
    ASSERT_TRUE(query::term_query(idx, term, &got).ok()) << term;
    EXPECT_EQ(got, expect) << term;
...
TEST(PostingInterleave, DocsOnlyWithPodRefTierRecovery) {
  SniiIndexInput in = BaseInput(1, "tag", IndexConfig::kDocsOnly);
  in.doc_count = 4096;
  std::vector<uint32_t> wide_docs;
  for (uint32_t d = 0; d < 2000; ++d) wide_docs.push_back(d);  // windowed pod_ref
...
TEST(PostingInterleave, SlimPodRefInterleaveAndPositionsRoundTrip) {
  SniiIndexInput in = BaseInput(1, "body", IndexConfig::kDocsPositions);
  in.doc_count = 30000;
  std::vector<uint32_t> docs;
  for (uint32_t d = 0; d < 500; ++d) docs.push_back(d * 50 + 1);  // df=500 (<512), spread
