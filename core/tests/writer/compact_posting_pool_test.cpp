...
TEST(CompactPostingPool, CursorOverLargeBudgetSelfTerminates) {
  CompactPostingPool pool;
  ...
  for (uint8_t b : decoy_bytes) decoy.put(&pool, b);
...
TEST(CompactPostingPool, AllocRunExactBlockFillStartsFreshBlock) {
  const uint32_t block = CompactPostingPool::kBlockSize;
  ...
  for (uint32_t i = 0; i < pad; ++i) {
    Chain c;
    c.start(&pool);
    const uint8_t b = static_cast<uint8_t>(0x40 + (i & 0x3F));
    c.put(&pool, b);
    chains.push_back(c);
    data.push_back({b});
  ...
  uint32_t guard = 0;
  while (g.level < grow_to_level) {
    const uint8_t b = static_cast<uint8_t>(0x80 + (gbytes.size() & 0x3F));
    g.put(&pool, b);
    gbytes.push_back(b);
