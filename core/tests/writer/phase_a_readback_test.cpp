...
uint64_t DocLen(uint32_t d) { return 2 + (d % 250); }
...
uint8_t WindowMinNorm(const std::vector<uint8_t>& norms, uint32_t start, uint32_t end) {
  ...
  for (uint32_t d = start; d < end; ++d) best = std::min(best, norms[d]);
  return best;
}

}  // namespace

TEST(PhaseAReadBack, DocsPrefixTilesAndMaxNormTightAndQueriesAgree) {
  ...
  for (uint32_t w = 0; w < prelude.n_windows(); ++w) {
    ...
    if (m.max_norm != 0) any_nonzero_max_norm = true;
