...
// Identifies the SNII container and carries basic compatibility info so a reader
// can fail fast before touching any streamed section or the tail meta region.
//
// On-disk layout (all multi-byte fields little-endian, fixed width; NOT framed by
// SectionFramer because it must be parseable without prior knowledge of the file):
//   u32 magic              == kContainerMagic
//   u16 format_version      == kFormatVersion
//   u16 min_reader_version  readers with kFormatVersion < this MUST refuse to read
//   u32 flags               container-level feature flags
//   u32 header_length       total bytes of this header including the checksum
//   u8  tail_pointer_size   size of the fixed tail pointer at EOF (hint for the reader)
//   u32 header_checksum     crc32c over all preceding header bytes
...
// Serializes the header to sink: writes header_length = kBootstrapHeaderSize and
// appends a crc32c over all preceding bytes. The caller's header_length field is
// ignored on input (it is always derived). Returns OK.
