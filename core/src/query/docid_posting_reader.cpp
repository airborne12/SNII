...
Status decode_flat_docs(const DictEntry& entry, Slice dd_region,
...
uint64_t prelude_abs(const LogicalIndexReader& idx, const DictEntry& entry,
                     uint64_t frq_base) {
  return idx.section_refs().posting_region.offset + frq_base + entry.frq_off_delta;
}

Status validate_windowed_prelude(const DictEntry& entry) {
  ...
  if (entry.prelude_len > entry.frq_len) {
    return Status::Corruption("docid_posting_reader: prelude_len exceeds frq_len");
  }
  return Status::OK();
}
...
Status open_window_preludes(const snii::io::BatchRangeFetcher& fetcher,
                            std::vector<WindowPlan>* plans) {
  for (WindowPlan& plan : *plans) {
    SNII_RETURN_IF_ERROR(
        FrqPreludeReader::open(fetcher.get(plan.prelude_handle), &plan.prelude));
  }
  return Status::OK();
}

Status plan_window_docs(const LogicalIndexReader& idx, WindowPlan* plan,
                        snii::io::BatchRangeFetcher* fetcher) {
  const ResolvedDocidPosting& posting = *plan->posting;
  plan->windows.reserve(plan->prelude.n_windows());
  for (uint32_t w = 0; w < plan->prelude.n_windows(); ++w) {
    snii::reader::WindowAbsRange range;
    SNII_RETURN_IF_ERROR(snii::reader::windowed_window_range(
        idx, posting.entry, posting.frq_base, posting.prx_base, plan->prelude, w,
        /*want_positions=*/false, /*want_freq=*/false, &range));
    WindowHandle handle;
    SNII_RETURN_IF_ERROR(plan->prelude.window(w, &handle.meta));
    handle.handle = fetcher->add(range.dd_off, range.dd_len);
    plan->windows.push_back(handle);
  }
  return Status::OK();
}
...
Status decode_window_plan(const snii::io::BatchRangeFetcher& fetcher,
                          const WindowPlan& plan, std::vector<uint32_t>* out) {
  for (const WindowHandle& window : plan.windows) {
    std::vector<uint32_t> docs;
    std::vector<uint32_t> freqs;
    std::vector<std::vector<uint32_t>> positions;
    SNII_RETURN_IF_ERROR(snii::reader::decode_window_slices(
        window.meta, fetcher.get(window.handle), Slice(), Slice(),
...
Status read_docid_posting(const LogicalIndexReader& idx, const DictEntry& entry,
...
Status read_docid_postings_batched(const LogicalIndexReader& idx,
  ...
  snii::io::BatchRangeFetcher prelude_fetcher(idx.reader());
  ...
  for (size_t i = 0; i < postings.size(); ++i) {
    ...
    if (posting.entry.enc == DictEntryEnc::kWindowed) {
      SNII_RETURN_IF_ERROR(validate_windowed_prelude(posting.entry));
      WindowPlan plan;
      plan.out_index = i;
      plan.posting = &posting;
      plan.prelude_handle =
          prelude_fetcher.add(prelude_abs(idx, posting.entry, posting.frq_base),
                              posting.entry.prelude_len);
  ...
  if (prelude_fetcher.pending() > 0) {
    SNII_RETURN_IF_ERROR(prelude_fetcher.fetch());
    SNII_RETURN_IF_ERROR(open_window_preludes(prelude_fetcher, &window_plans));
  }
  ...
  for (WindowPlan& plan : window_plans) {
    SNII_RETURN_IF_ERROR(plan_window_docs(idx, &plan, &docs_fetcher));
  }
  if (docs_fetcher.pending() > 0) SNII_RETURN_IF_ERROR(docs_fetcher.fetch());

  for (const FlatPlan& plan : flat_plans) {
    SNII_RETURN_IF_ERROR(
        decode_flat_plan(docs_fetcher, plan, &(*docids)[plan.out_index]));
  }
  for (const WindowPlan& plan : window_plans) {
    SNII_RETURN_IF_ERROR(
        decode_window_plan(docs_fetcher, plan, &(*docids)[plan.out_index]));
  }
  return Status::OK();
}
