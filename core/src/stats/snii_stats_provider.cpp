...
Status LookupEntry(const snii::reader::LogicalIndexReader& idx,
                   std::string_view term, bool* found, DictEntry* entry) {
  uint64_t frq_base = 0;
  uint64_t prx_base = 0;
...
Status SniiStatsProvider::open(const snii::reader::LogicalIndexReader* idx,
  ...
  SNII_RETURN_IF_ERROR(
      NormsPodReader::open(Slice(out->norms_bytes_), &out->norms_reader_));
  out->has_norms_ = true;
  return Status::OK();
}
...
Status SniiStatsProvider::total_term_freq(std::string_view term,
                                          uint64_t* ttf) const {
