...
AwsApiGuard::AwsApiGuard() { api_acquire(); }
AwsApiGuard::~AwsApiGuard() { api_release(); }
...
Status S3FileReader::open(const S3Config& cfg, const std::string& key,
                          S3FileReader* out) {
...
Status S3FileReader::read_at(uint64_t offset, size_t len,
