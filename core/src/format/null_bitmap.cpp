...
NullBitmapWriter::NullBitmapWriter()
    : bitmap_(std::make_unique<roaring::Roaring>()) {}

NullBitmapWriter::~NullBitmapWriter() = default;

void NullBitmapWriter::add_null(uint32_t docid) { bitmap_->add(docid); }
...
void NullBitmapWriter::finish(uint32_t doc_count, ByteSink* sink) const {
  ...
  payload.put_bytes(Slice(reinterpret_cast<const uint8_t*>(roaring_buf.data()),
                          roaring_size));
...
Status NullBitmapReader::open(Slice framed, NullBitmapReader* out) {
  ...
  SNII_RETURN_IF_ERROR(
      payload.get_bytes(static_cast<size_t>(roaring_size), &roaring_bytes));
  ...
  const size_t probed = roaring_bitmap_portable_deserialize_size(
      rb, static_cast<size_t>(roaring_size));
