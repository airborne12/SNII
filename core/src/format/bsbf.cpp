...
const uint32_t kBsbfSalt[kBsbfBitsSetPerBlock] = {
...
__attribute__((target("avx2"))) __m256i mask_avx2(uint32_t key) {
  const __m256i salt = _mm256_setr_epi32(
      static_cast<int>(kBsbfSalt[0]), static_cast<int>(kBsbfSalt[1]),
...
__attribute__((target("avx2"))) bool find_avx2(const uint32_t* words, uint32_t block, uint32_t key) {
...
Status BsbfBuilder::serialize(ByteSink* sink) const {
  ...
  hdr[0] = 'B'; hdr[1] = 'S'; hdr[2] = 'B'; hdr[3] = 'F';
  hdr[4] = 1;  // version
...
Status bsbf_probe(snii::io::FileReader* reader, const BsbfHeader& header, uint64_t hash,
  ...
  if (blk.size() < kBsbfBytesPerBlock)
    return Status::Corruption("bsbf: short block read");
