...
// out, and fills meta (mode/uncomp_len/disk_len/crc). The region carries no header.
Status emit_region(Slice plain, int level, ByteSink* out, FrqRegionMeta* meta) {
  ...
  if (should_compress(level, plain.size())) {
    ...
    SNII_RETURN_IF_ERROR(
        zstd_compress(plain, level > 0 ? level : kDefaultZstdLevel, &disk));
  } else {
    meta->zstd = false;
...
// Materializes a region's plaintext (raw borrows the view; zstd decompresses) and
// verifies its crc + slice length against meta.
Status open_region(Slice disk, const FrqRegionMeta& meta, std::vector<uint8_t>* holder,
  ...
  SNII_RETURN_IF_ERROR(
      zstd_decompress(disk, static_cast<size_t>(meta.uncomp_len), holder));
  *plain = Slice(*holder);
  return Status::OK();
}
