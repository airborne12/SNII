...
void TailMetaRegionBuilder::finish(ByteSink* sink) const {
  ...
  const uint64_t meta_region_len =
...
Status TailMetaRegionReader::open(Slice region, TailMetaRegionReader* out) {
  ...
  if (directory_offset + directory_length > region.size() ||
...
Status TailMetaRegionReader::find(uint64_t index_id, std::string_view suffix,
                                  bool* found, Slice* per_index_meta_bytes) const {
  LogicalIndexRef ref;
