...
Status decode_header(Slice block, ByteSource* src, uint64_t* index_id,
  ...
  suffix->assign(reinterpret_cast<const char*>(suffix_view.data()),
                 suffix_view.size());
  return Status::OK();
}
...
PerIndexMetaBuilder::PerIndexMetaBuilder(uint64_t index_id,
                                         std::string index_suffix,
                                         uint32_t flags)
    : index_id_(index_id),
      index_suffix_(std::move(index_suffix)),
      flags_(flags) {}

void PerIndexMetaBuilder::set_stats(const StatsBlock& stats) { stats_ = stats; }

void PerIndexMetaBuilder::set_sampled_term_index(Slice framed_bytes) {
  sampled_term_index_.assign(framed_bytes.data(),
                             framed_bytes.data() + framed_bytes.size());
}

void PerIndexMetaBuilder::set_dict_block_directory(Slice framed_bytes) {
  dict_block_directory_.assign(framed_bytes.data(),
                               framed_bytes.data() + framed_bytes.size());
}
...
void PerIndexMetaBuilder::add_raw_section(Slice framed_bytes) {
  extra_sections_.emplace_back(framed_bytes.data(),
                               framed_bytes.data() + framed_bytes.size());
}
...
Status PerIndexMetaReader::open(Slice block, PerIndexMetaReader* out) {
  ...
  SNII_RETURN_IF_ERROR(decode_header(block, &src, &out->index_id_,
                                     &out->index_suffix_, &out->flags_));
