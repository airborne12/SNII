...
// Number of bytes covered by header_checksum: everything except the trailing crc32c.
constexpr size_t kChecksumCoverage = kBootstrapHeaderSize - 4;

// Writes all fixed fields except the trailing checksum. Field order is the on-disk
// contract; reuse ByteSink fixed-width primitives, never hand-assemble bytes.
...
Status decode_bootstrap_header(Slice data, BootstrapHeader* out) {
  ...
  // Reject any size other than the exact fixed header: short input is truncation,
  // longer input means stray trailing bytes the parser would otherwise ignore.
  ...
  const auto min_reader_version =
      static_cast<uint16_t>((version_pair >> 16) & 0xFFFFu);
  ...
  out->magic = magic;
  out->format_version = static_cast<uint16_t>(version_pair & 0xFFFFu);
