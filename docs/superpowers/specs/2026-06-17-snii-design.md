...
- 倒排：`.frq` POD（自适应窗口 256/512/1024/2048、列式 dd+freq、PFOR、win_base 可 byte-copy stitch）、列式 prelude（窗口元信息 + 可选 super-block 目录）、`.prx` POD（positions，ZSTD）。
...
- **`.prx` 窗口**：独立分窗，与 `.frq` 在 256-doc unit 边界可对齐；header `codec/uncomp_len/comp_len/crc32c`；默认对足够大窗口 ZSTD；超大单 doc positions 退化为单大窗口。
...
- **phrase query**：计划阶段同时安排命中 doc/freq + positions 读取，避免位置判断阶段再触发串行小读。
