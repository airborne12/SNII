...
5. final k-way merge 后顺序写 data sections（spill-only：无 section 临时文件）
   posting region 直写容器输出：每个 pod_ref term 按 term 顺序，将其 [prx span][frq span]
...
### 总体布局
```plaintext
posting region（单一交错 sink，每索引一个）:
  ...
  !has_prx（docs-only/keyword 档）时 prx span 为空，posting region 仅含 frq span。
  reader 用同一区的 frq_base / prx_base 加 off_delta 解析两段的绝对偏移：
    abs = posting_region.offset + base + off_delta（prx 在 frq 之前）。
...
- 通过 per-index meta 定位 DICT region、posting region（含每 term 的 prx span / frq span）、norms POD 和 null bitmap POD。
...
PerIndexMetaHeader:
  meta_format_version   # = 1（从零、pre-launch 的单一格式；该字段是自描述 fail-fast
                        #   哨兵，per-index meta header 与 tail meta region 共用此常量，
                        #   reader 对不匹配返回 Corruption。仅在 launch 后需与已写索引
                        #   共存时才 bump）
  index_id
  index_suffix
