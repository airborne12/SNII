...
### 短语解码成本（perf 实测定位，待优化）

本地短语 SNII 慢于 CLucene 是 **CPU / 数据结构问题，非 I/O**（`perf record` self-time，无任何 read 热点）：
...
**已实施优化**（`core/src/query/phrase_query.cpp` + `prx_pod` CSR 解码）：
1. **两阶段惰性**：所有词（含驱动词）先 **docid-only 求交**，位置**仅为最终幸存候选**解码；Phase B 按窗口 `last_docid` 跳过无候选窗口。
2. **CSR 扁平位置**：新增 `read_prx_window_csr`（位置解进 `pos_flat`+`pos_off` 扁平 buffer，消除嵌套 `vector<vector>` 的逐 doc malloc）。

**实测（50M，median，short phrase 'failed to place order'，1.36M 命中）**：

| 版本 | SNII 短语 | vs 原始 | vs CLucene(514ms) |
|---|---|---|---|
| 原始（vector<vector>）| 1973ms | — | 3.8× 慢 |
| + 两阶段惰性 | 1661ms | 1.19× | — |
| **+ CSR 扁平位置** | **1108ms** | **1.78× 快** | **2.15× 慢** |

正确性 `ALL DOCIDS IDENTICAL` + 352 core 测试不回归；金指标（串行轮次 7 / range_gets 14）与 I/O 不变（零 S3 回归）。**剩余差距是架构性的**（SNII 物化候选位置 vs CLucene skip-list 纯流式），追平本地需短语流式求值（大重构）；**S3 上 SNII 短语已快 1.7×**（RTT 主导），本地差距不影响对象存储场景。
