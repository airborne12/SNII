...
## 第二轨：真实 OSS 端到端 wall-clock（已实测）

SNII 读路径经 `snii::io::S3FileReader`（aws-sdk-cpp，virtual-hosted 寻址）直接从 `oss://doris-community-test`（阿里云香港，外网 endpoint）做 Range GET 查询；`MeteredFileReader` 同时记录成本模型四指标。批量读经 `read_batch` 并发 GET。索引先用 `S3FileWriter` 上传到 OSS。复现见 `examples/oss_latency_demo.cpp`。

实测（SNII 冷查询，从真实 OSS 读取）：

| 规模（索引大小） | TERM(high-df) wall / 轮次 | 5-term PHRASE wall / 轮次 / GET |
|---|---|---|
| 200K（6.5MB） | 18ms / 2 | 137ms / 2 / 2 |
| 1M（33MB） | 27ms / 2 | **321ms / 5 / 9** |

**校准与验证**：1M 短语 5 串行轮次、321ms → ≈ **64ms/轮**，与早先 ossutil 实测 OSS 单次往返（~50–130ms）一致 —— **真实 wall-clock 验证了"串行轮次 × RTT"模型**。

**对比投影**（用实测 ~64ms/轮）：1M 规模 5-term 短语冷查关键路径延迟
- SNII：5 轮 → **321ms 实测**
- CLucene：成本模型 11 轮 × 64ms ≈ **~700ms 投影**  → 约 **2.2×** 更低延迟。

CLucene 侧的真实 OSS wall-clock 需为 CLucene 实现一个 S3-backed `Directory`（较重），本期以"成本模型轮次 × 实测 SNII 单轮延迟"投影；SNII 侧已实测端到端。S3 后端 round-trip 另有单测 `S3ObjectStoreTest.RoundTrip` 对真实 OSS 验证通过。
