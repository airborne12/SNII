...
**诚实的内存↔CPU 权衡（Pareto）**：bound 内存需 spill，spill 有 CPU 成本。SNII 的 build 资源**可调**（spill 阈值），CLucene 不可调（其 5M 峰值 ~716MiB 由 `optimize()` 合并决定，设大 RAMBuffer 也不变）。SNII 可在**任一轴上反超** CLucene：

| SNII 配置 (5M) | SNII CPU / 内存 | CLucene |
|---|---|---|
| 选低内存 (spill=128) | 24.5s / **665MiB** ✅内存胜 | 18.5s / 715MiB |
| 选低 CPU (spill=0) | **14.6s** ✅CPU胜 / 1330MiB | 18.0s / 716MiB |

但因 spill 往返开销，SNII 无法在同一配置同时让两轴都低于 CLucene 单点——这是 out-of-core 构建的固有 Pareto 权衡，二者在该前沿上很接近。查询侧 SNII 大幅领先（轮次/字节/冷查延迟 5–11×）的 build 代价已从「6-8×CPU、11×内存」优化到「内存持平/更优、CPU 1.33×（或 CPU 更优、内存更高）」。
