...
**1M docs 实测**（CLucene 与 SNII 均真实 OSS 测量）：

| 查询 | CLucene wall / 轮次 / GET | SNII wall / 轮次 / GET | wall 比 |
|---|---|---|---|
| TERM high-df ('wa', df=933K) | 103.2ms / 2 / 2 | **21.7ms / 2 / 2** | **CLucene/SNII = 4.9×** |
| TERM mid-df | 7.9ms | 6.9ms | 1.17× |
| TERM low-df | 8.5ms | 5.7ms | 1.60× |
| **5-term PHRASE** | **357ms / 11 / 11** | **265ms / 5 / 8** | **1.35×（轮次 2.2×）** |

**解读**：
- 高频 term **SNII 真实快 4.9×**：CLucene cursor 把大倒排表拆成 ~10 次小顺序读，每次付真实 OSS 往返；SNII 2 次批量并发读。真实 wall-clock 比成本模型轮次（2 vs 2）更能体现 cursor 劣势。
- 5-term 短语 **SNII 快 1.35×、轮次 2.2× 更少**：SNII 前置规划把 .frq/.prx range 合进 5 轮批量并发，CLucene 逐段依赖 11 轮。SNII 多读的字节（12.6MB vs 11.5MB）部分抵消，但轮次节省净胜。
- 小规模（5K，索引 <1MiB）SNII 整读单块的字节开销会盖过轮次节省（短语 85ms vs 60ms）——故以 1M+ 为准；这是真实测得的权衡，非投影。

成本模型轨（前文）与真实 OSS 轨结论一致：SNII 通过批量并发规划降低对象存储上的串行 I/O 轮次/请求数，换取更低的冷查延迟，代价是更高的读取字节数（符合设计文档取舍）。
